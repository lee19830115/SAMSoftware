...
// The DMA set up and double-buffering framework
...
void 	processAudioCallback( void ) {
	...
	for (i=0;i<AUDIO_BLOCK_SIZE;i++) {
		...
		// Bring new audio in from ADCs to SHARC core 1 for processing
		multiCoreAudioCtrl->Core_1_Audio_In_L[i] = ADAU1979_Jack_In_Left[i];
		multiCoreAudioCtrl->Core_1_Audio_In_R[i] = ADAU1979_Jack_In_Right[i];

		// Send what we processed last frame over to SHARC core 2
		...
		// And copy the audio back from core 2 from the last frame and send to DACs
		ADAU1966_Jack_Out_P20_Left[i]  = multiCoreAudioCtrl->Core_2_Audio_Out_L[i];
		ADAU1966_Jack_Out_P20_Right[i] = multiCoreAudioCtrl->Core_2_Audio_Out_R[i];

		ADAU1966_Jack_Out_P19_Left[i]  = multiCoreAudioCtrl->Core_2_Audio_Out_L[i];
		ADAU1966_Jack_Out_P19_Right[i] = multiCoreAudioCtrl->Core_2_Audio_Out_R[i];
	...
	*pREG_SEC0_RAISE = INTR_SOFT7;
	...
	multiCoreAudioCtrl->audioLevelDb = calculateAudioLevels( ADAU1979_Jack_In_Left );
...
void 	setup() {
	...
	multiCoreAudioCtrl->audio_channel_count = ADAU1452_SLOTS;
